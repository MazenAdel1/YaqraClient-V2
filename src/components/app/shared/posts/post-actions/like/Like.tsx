...
export default function Like({ postId, likeCount, isLiked }: LikeProps) {
  ...
  const { mutateAsync } = useMutation({
    ...
    onMutate: async () => {
      ...
      // Optimistically update all queries containing this post
      queryClient.setQueriesData<{ id: number; likeCount: number; isLiked: boolean }[]>(
        { predicate: (query) => Array.isArray(query.state.data) },
        (oldData) => {
          if (!oldData) return oldData;
          return oldData.map((item) =>
            item.id === postId
              ? { ...item, likeCount: isLiked ? item.likeCount - 1 : item.likeCount + 1, isLiked: !isLiked }
              : item
          );
        }
      );
